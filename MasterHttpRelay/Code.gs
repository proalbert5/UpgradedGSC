...
function _doBatch(items) {
      ...
      for (var j = 0; j < fetchArgs.length; j++) {
        ...
        try {
          ...
          for (var key in fallbackReq) {
            if (key !== "url") fallbackOpts[key] = fallbackReq[key];
          };
  ...
  for (var i = 0; i < items.length; i++) {
    if (errorMap[i]) {
      ...
      if (!resp) results.push({ e: "fetch failed" });
      else results.push({
...
function _buildOpts(req, simple) {
  ...
  opts.headers = simple ? _buildSimpleHeaders(req.h) : _buildFullHeaders(req.h);
  if (req.b) {
...
function _buildFullHeaders(custom) {
  var p = _getProfile();
  var headers = {
    "User-Agent": p.ua,
  ...
  if (custom && typeof custom === "object") {
    for (var k in custom) {
      if (!_shouldSkipHeader(k)) headers[k] = custom[k];
    };
  };
  return headers;
};

function _buildSimpleHeaders(custom) {
  var headers = {
